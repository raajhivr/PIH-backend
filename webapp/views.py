...
def all_products(requests):
    try:
        if requests.method=="POST":
            try: 
                all_products_column=[["SPEC-ID","SPEC*"],["NAM PROD","NAM*"],["MATERIAL NUMBER","MAT*"],["BDT","BDT*"],["CAS NUMBER","CAS*"]]
                selected_categories=["BDT","NAM","CAS","MAT","SPEC"]
                matched_categories = [["BDT","BDT","BDT*"],["NAM","NAM PROD","NAM*"],["MAT","MATERIAL NUMBER","MAT*"],["CAS","CAS NUMBER","CAS*"],["SPEC","SPEC-ID","SPEC*"]]
                data=''
                data_json=''
                search=''
                search_split=''
                ...
                if len(search)>=3 and (search_key.upper() in selected_categories or search.upper() in selected_categories):
                    ...
                    all_product_list=[]
                    for item,match,key in matched_categories:
                        if search_key.upper()==item or search.upper()==item:
                            ...
                            for value in column_value:
                                out_dict={"name":str(value).strip(),"type":match,"key":key}
                    ...
                    for column,key in all_products_column:
                        ...
                        for item in column_value:
                            out_dict={"name":str(item).strip(),"type":column,"key":key}
                            all_product_list.append(out_dict)  
    ...
    try:
        if requests.method=="POST":
            try:
                ...
                data_json=''
                column_add=[]
                all_product_column=all_products_column=[["SPEC-ID","SPEC*"],["NAM PROD","NAM*"],["MATERIAL NUMBER","MAT*"],["BDT","BDT*"],["CAS NUMBER","CAS*"]]
                data = requests.body.decode('utf-8')
                data_json=json.loads(data)
                ...
                for column,key in all_product_column:
